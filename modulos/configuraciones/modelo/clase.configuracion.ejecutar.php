...
class Ejecucion {
	...
	*/
	...
	function __construct() {
        ...
        {
             $this -> configuracionid = (is_numeric($int) && $int > 0)?"= $int":"> 0";
        }
		...
		if ($int < $this -> respuestasCont) {
			switch($FORMAT) {
				case "HTML" :
					return htmlentities($this -> respuestaDesc[$int], (int)$this -> flag, "Windows-1252", true);
				case "INPUT" :
					return htmlspecialchars_decode(htmlspecialchars(htmlentities($this -> respuestaDesc[$int], (int)$this -> flag, "Windows-1252", true)), ENT_NOQUOTES);
				default :
        ...
        public function GET_ITEMENUNCIADO($FORMAT = "NORMAL") {
            switch($FORMAT) {
                    case "HTML" :
                            return htmlentities($this -> itemEnunciado, (int)$this -> flag, "Windows-1252", true);
                    case "INPUT" :
                            return htmlspecialchars_decode(htmlspecialchars(htmlentities($this -> itemEnunciado, (int)$this -> flag, "Windows-1252", true)), ENT_NOQUOTES);
                    default :
