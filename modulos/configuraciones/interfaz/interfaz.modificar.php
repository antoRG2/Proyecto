...
$objConf -> set_ID($ConfID);
$objConf -> _QUERY();
$objConf ->_QUERY4ITEMS();
?>
<!DOCTYPE html>
<html lang="es">
	<head>
	    ...
	    <link rel="stylesheet" type="text/css" 
            href="../../../CSS/crearConfiguraciones.css">
	    <link rel="stylesheet" type="text/css" 
	     ...
	     <script>
	     	...
	     	$(function () {
				...
				$("#btnGuardar").click(function(){
					...
					var request = jQuery.ajax({
						...
						data: "id=<?=$ConfID?>&editar=1&dataJSON=" + dataJSON
                            + '&nombre=' + encodeURIComponent($('#nombre').val())
                            + '&descripcion=' +
                            encodeURIComponent($('#descripcion').val()) + '&pub=' + pub,//data
						async:false
					});
			...
			{			
				...
				var request = jQuery.ajax({
					...
					data: 'id=<?=$ConfID?>&selected='+$("#filtro").val(),//data
					async:false
				});
				...
				switch(selected) {
				    ...
				    case "Todos":
				        var catalog = '<div id="catalog"><h3><a href="#">' +
                            'Todos los items</a></h3><div><ul>';
				    ...
				    case "Dificultad":
				        ...
				        for (i = 0; i < json.length; i++) {
				   		    if(i == 0)
				   		    {
				   		        catalog += '<h3><a href="#">' + json[i]['dificultad']
                                    + '</a></h3><div><ul><li id="' + json[i]['id']
                                    + '">' + json[i]['nombre'] + '</li>';
				   		    }
				   		    else{
				 	    ...
				 	    for (i = 0; i < json.length; i++) {
				   		    if(i == 0) {
				   		        ...
				   		        if (json[i]['area'] == json[i - 1]['area']) {
				   		            ...
				   		            catalog += '</ul></div><h3><a href="#">' + json[i]['area']
                                        + '</a></h3><div><ul><li id="' + json[i]['id'] + '">'
                                        + json[i]['nombre'] + '</li>';
				   		        }
				   		    }
    ...
    <body>
	    ...
	    <div class="transBack">
            <div class="displayData">
                <div class="title">
                    <h2>Modificar Configuraci&oacute;n</h2>
                </div>			
                <br />
                ...
                <form name="FRMItems" id="FRMItems" method="post" 
                    ...
                    <table>
	                    <tr>
		                    ...
		                    <td><input type="text" name="nombre" id="nombre" style="width: 500px;" value="<?=$objConf ->get_Nombre(0,"INPUT")?>"/></td>
	                    </tr>
	                    <tr>
		                    <td><label for="descripcion">Descripci&oacute;n:</label></td>
		                    <td><textarea id="descripcion" name="descripcion" style="width: 500px;height: 50px;"><?=$objConf ->get_Descripcion(0,"INPUT")?></textarea></td>
	                    </tr>
	                    <tr>
	                ...
	                <div id="drop">
		                ...
		                <div class="ui-widget-content">
			                <ol>
				                <?php 
				                for($i = 0; $i < $objConf->get_ContadorItems();
                                            $i++){?>
					                <li id="<?=$objConf -> get_ItemsID($i)?>">
                                    <?=$objConf -> get_ItemsName($i,"HTML")?></li>
				                <?php }?>
			                </ol>
		                </div>
	                </div>
                    ...
                    <div class="controlbuttonsDiv">
	                    <div style="float:left"><input type="button" 
                            name="btnLimpiar" id="btnLimpiar" value='Limpiar'/></div>
	                    <div style="float:right"><input type="button" 
                            name="btnCancelar" id="btnCancelar" value='Cancelar'/></div>
	                    <div style="float:right"><input type="button" 
                            name="btnGuardar" id="btnGuardar" value='Guardar'/></div>						
                    </div>
                    <!--</center>-->
			    </form>
            </div>
