			 ...
			 $(function() {
				...
				$("#btnGuardar").click(function(){
					...
					var request = jQuery.ajax({
						...
						data: "dataJSON=" + dataJSON + '&nombre=' +
                            encodeURIComponent($('#nombre').val()) +
                            '&descripcion=' + encodeURIComponent(
                            $('#descripcion').val()) + '&pub=' + pub,//data
						async:false
					});
				       ...
				       var catalog= '<div id="catalog">';
				   	    for(i = 0; i < json.length; i++){
				   	        ...
				   	        if (i == 0) {
				   		        ...
				   		        catalog += '<h3><a href="#">' + json[i]
                                ['dificultad'] + '</a></h3><div><ul><li id="'
                                + json[i]['id'] + '">' + json[i]['nombre'] + '</li>';
				   		    }
				   		    else{

				   	            if (json[i]['dificultad'] ==
                                    json[i - 1]['dificultad']) {
        ...
        <form name="FRMItems" id="FRMItems" method="post" 
            ...
            <table>
                <tr>
	                ...
	                <td><input type="text" name="nombre" id="nombre" style=
                        "width: 500px;"  /></td>
                </tr>
                <tr>
	                <td><label for="descripcion">Descripci&oacute;n:</label></td>
	                <td><textarea id="descripcion" name="descripcion" style=
                        "width: 500px;height: 50px;"></textarea></td>
                </tr>
                <tr>
	                <td><label for="filtro">Filtrar por:</label></td>
	                <td><select name="filtro" id="filtro" 
                ...
                <tr>
	                <td><label for="publica">¿La configuraci&oacute;n 
                        es p&uacute;blica?</label></td>
            ...
            <div class="controlbuttonsDiv">
	            <div style="float:left"><input type="button" name="btnLimpiar" 
                    id="btnLimpiar" value='Limpiar'/></div>
	            <div style="float:right"><input type="button" name="btnCancelar"
                     id="btnCancelar" value='Cancelar'/></div>
	            <div style="float:right"><input type="button" name="btnGuardar"
                     id="btnGuardar" value='Guardar'/></div>						
            </div>
            <!--</center>-->
        </form>	
	</body>
