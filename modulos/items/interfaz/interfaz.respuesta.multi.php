...
<?php for($i = 1; $i <= $cantR; $i++){?>
		<tr id='<?=$i?>'>
			<td><input type="file" name="respuesta[]" id="respuesta<?=$i?>" size="80"/></td>
			<td><input type="checkbox" name="checkbox[]" class="chk" id="chk_<?=$i?>" /><input type="hidden" name="acierto[]" id="acierto<?=$i?>" value="0"/></td>
		</tr>
<?php }?>
		</tbody>
	</table>
...
<?php for($i = 1; $i <= $cantR; $i++){?>
		<tr id='<?=$i?>'>
			<td><input type="text" name="respuesta[]" id="respuesta<?=$i?>" size="80"/></td>
			<td><input type="checkbox" name="checkbox[]" class="chk" id="chk_<?=$i?>" /><input type="hidden" name="acierto[]" id="acierto<?=$i?>" value="0"/></td>
		</tr>
<?php }?>
		</tbody>
