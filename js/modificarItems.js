...
jQuery(function($) {
    ...
    $(".scheck").change(function(){
        ...
        if ($('#tipo').val() > 0 && $('#clasificacion').val() > 0) {
            ...
            if ($("#nRespuestas").spinner("value") > 0)
    	    {
    	        ...
    	        $('#areaRespuestas')
                    .load('interfaz.respuesta.multi.php?clasificacion=' +
                    $('#clasificacion').val() + '&cantR=' +
                    $("#nRespuestas").spinner("value"));
    	    }
    	    else{
    		    $('#areaRespuestas').html("");
    	    }
        ...
        if (contRespuestas < $(this).spinner("value")) {
	  		if($('#tbRespuestas') != null)
	  		{
	  		    $("#tbRespuestas tr:eq(0)").clone().removeClass('fila-base')
                    .show().attr("id", $(this).spinner("value")).appendTo("#tbRespuestas");

	  		    $("#tbRespuestas tr:last input[type=checkbox]")
                    .attr("id", "chk_" + $(this).spinner("value"));
	...
	$("#btnGuardar").click(function(){
		...
		if (INITTYPEEN == 1) {
		    if (($.trim(en1) == '' || en1.length == 0) && ($.trim(en1T) == ''
                || en1T.length == 0) && ($.trim(en2) == '' || en2.length == 0)
                && ($.trim(en3) == '' || en3.length == 0)) {
	...
	$(document).on("click",".eliminarR",function(){
		...
		var request = jQuery.ajax({
				...
				data: {
				    'id': id, 'itemID': itemID, 'descripcion': descripcion,
